...
const float MAX_RADIUS = 0.03;             // Max radius in normalized coords
const float RING_THICKNESS = 0.015;        // Ring width in normalized coords
const float CURSOR_WIDTH_CHANGE_THRESHOLD = 0.5; // Triggers ripple if cursor width changes by this fraction
vec4 COLOR = vec4(sRGBToLinear(iCurrentCursorColor.rgb), 0.6); // uses cursor's theme color at 60% opacity
const float BLUR = 3.0;                    // Blur level in pixels
const float ANIMATION_START_OFFSET = 0.05; // Start the ripple slightly progressed (0.0 - 1.0)
